...
public class Main {
	...
	public static final String CREATE_TABLE_BOOK_QUERY = "CREATE TABLE book(id INT PRIMARY KEY AUTO_INCREMENT, "
			+"person_id INT, title VARCHAR(255))";
	
	public static final String ADD_CONSTRAINT_BOOK_PERSON = "ALTER TABLE book ADD CONSTRAINT book_person FOREIGN KEY"
	+" (person_id) REFERENCES person(id)";
	
	public static final String INSERT_PERSON = "INSERT INTO person(name, age) VALUES (?, ?)";
	
	public static final String INSERT_BOOK = "INSERT INTO book(title, person_id) VALUES (?, ?)";
	
	public static final String SELECT_PERSON_BOOK = "SELECT p.name, p.age, b.title FROM person p JOIN book b "
			+"ON b.person_id=p.id"; 
...
//			statement.setString(1, "Chutanka");
//			statement.setInt(2, 1);
//			statement.executeUpdate();
//			statement.close();
			...
			while(rs.next()){
				System.out.println(rs.getString("name")+" "+rs.getInt("age")+" "+rs.getString("title"));
			}
